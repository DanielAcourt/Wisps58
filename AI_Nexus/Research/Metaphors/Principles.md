...
## 🎱 7. The Pool Cue Metaphor
*   **Metaphor:** If you make a high-precision pool cue and use it to beat people over the head rather than playing the game, is it the fault of the tool or the user?
*   **Principle:** **Symmetrical System Grounding vs. Prompt Hype.** Large Language Models are high-precision, low-entropy cognitive instruments. Beating people over the head with the tool corresponds to expecting raw, ungrounded models to act as self-navigating Oracles (leading to hallucinations and failure loops). Playing the game means anchoring the tool in the hands of the operator within a structured, deterministic system.
*   **Technical Application:** Implementing the **Symmetrical Guard** and the **Command SOP** (AAS Protocols). The AI is restricted from asserting physical status without executing direct measurement tools first, forcing the model to calculate angles rather than hallucinating outcomes.
