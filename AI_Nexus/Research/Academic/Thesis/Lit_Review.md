...
* 📁 **Seth Dobrin & Łukasz Chmiel / ARYA Labs (2026):** [`The Unfireable Safety Kernel: Execution-Time AI Alignment for AI Agents and Other Escapable AI Systems`](../Literature/The_Unfireable_Safety_Kernel_Execution_Time_AI_Alignment_for_AI_Agents.md)
  > <span style="color: #D32F2F; font-weight: bold;">[Daniel / Candidate Critique & Technical Refutation]:</span> *"Dobrin & Chmiel (2026) coin 'execution-time AI alignment' by wrapping AI agents in a Rust HTTP microservice. While their 4 control properties (process separation, structurally-only path, fail-closed, signed evidence) and Kani model checking are sound, claiming novelty ignores 37 years of computer science prior art (Rushby, 1989). Furthermore, their HTTP localhost proxy introduces 496ms p99 latency. PSTA achieves true execution-time alignment directly in bare-metal C++ memory space at microsecond speeds via Agency Arbitration Schema (AAS) and non-compensatory step guards ($\theta_i$)."*
