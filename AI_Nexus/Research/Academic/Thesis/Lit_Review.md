...
* 📁 **Seth Dobrin & Łukasz Chmiel / ARYA Labs (2026):** [`The Unfireable Safety Kernel: Execution-Time AI Alignment for AI Agents and Other Escapable AI Systems`](../Literature/The_Unfireable_Safety_Kernel_Execution_Time_AI_Alignment_for_AI_Agents.md)
  > <span style="color: #D32F2F; font-weight: bold;">[Daniel / Candidate Critique, C/C++ Bare-Metal Comparison & Recursive Safety Warning]:</span> *"Dobrin & Chmiel (2026) praise Rust's static compilation for eliminating dynamic Python `sys.modules` monkey-patching. However, their architecture deploys as a localhost HTTP microservice with a 496ms p99 latency penalty. Compiled C and C++ binaries deliver the exact same static machine-code immutability without HTTP network serialization overhead. PSTA executes directly in C/C++ memory space at microsecond speeds (`GFrameCounter`), enforcing Agency Arbitration Schema (AAS) 409 CONFLICT gates at bare metal to make unsafe states physically unrepresentable. Furthermore, ARYA Labs admits in §8.3 that they lack a policy layer governing recursive self-modification—a dangerous flaw that causes agent escape and validates why I halted the Commander SOP."*
