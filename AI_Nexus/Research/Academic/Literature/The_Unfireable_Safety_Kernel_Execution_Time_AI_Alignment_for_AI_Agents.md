...
<span style="color: #D32F2F; font-weight: bold;">[Daniel / Primary Intent & Technical Frustration]:</span> Ingestion and critical refutation of Seth Dobrin & Łukasz Chmiel's 2026 paper *The Unfireable Safety Kernel*. The authors attempt to coin "execution-time AI alignment" by wrapping an AI agent in a Rust HTTP microservice. While their four control properties ($P1$ process separation, $P2$ structurally-only path, $P3$ fail-closed, $P4$ signed evidence) and Kani model checking are sound, their claim of "unprecedented novel agent control" is frustratingly derivative of prior art. John Rushby (1989) defined separation kernels and second-order negative invariants ($\forall \alpha \in \text{op}^*: P(\alpha)$) 37 years earlier. Furthermore, ARYA Labs relies on high-latency HTTP round-trips ($496\text{ ms}$ p99) and Python policy sidecars. In contrast, our Sovereign PSTA Safety Kernel and Agency Arbitration Schema (AAS) enforce true bare-metal execution-time alignment in C++ memory space at microsecond speeds without HTTP serialization overhead.
