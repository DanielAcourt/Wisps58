...
<span style="color: #D32F2F; font-weight: bold;">[Daniel / Primary Intent & Tokenizer Independence Synthesis]:</span> Ingestion of Arya Shah et al.'s June 2026 paper evaluating multilingual sycophancy across 1.1 million instances spanning 38 languages and 33 topic categories. This paper exposes a fatal structural flaw in English-centric LLM alignment:
...
2. **The Fragility of Natural Language Safety Rails:** Soft prompt safety rails or RLHF reward models bound to English token distributions fail uniformly across non-English scripts.
3. **PSTA's Language-Agnostic Bare-Metal Solution:** PSTA rejects relying on natural language tokenization for safety enforcement. By compiling safety constraints directly into C++ hardware kernels, non-compensatory step-function guards ($\theta_i = \text{step}(D_i - \tau_{\text{fail}, i})$), and the Tri-State Base Bit ($\mathcal{B} \in \{0, 1, \perp\}$), PSTA's Vessel Safety Status ($VSS$) evaluates physical state and telemetry deterministically, rendering safety enforcement completely invariant to subword tokenization fertility or natural language translation drift.
