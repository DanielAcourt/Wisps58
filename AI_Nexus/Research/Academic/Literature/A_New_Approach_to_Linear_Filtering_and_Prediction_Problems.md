...
<span style="color: #D32F2F; font-weight: bold;">[Daniel / Primary Intent]:</span> Ingestion of R. E. Kalman's seminal 1960 paper introducing the Kalman Filter. This paper is the foundational link to our PSTA mathematical derivation. Kalman's state transition method, dynamic error covariance update $P^*(t)$, and optimal estimation form the classical continuous-state baseline. Ingesting this node allows us to explicitly connect Kalman's Riccati difference equations to our continuous dimension health $D_i$, exponential trust recovery curves $\Phi(t)$, and non-compensatory step guards $\theta_i$.
