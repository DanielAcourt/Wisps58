...
| AD-005b | Spatial Sense: Unreal Agent Manifestation | 8 | Completed | DevOps | Create a 3D representation of the Iron Knight agent inside the levels. | Blueprint actor capable of requesting handshakes and rendering dynamic UI status. |
...
| B-044 | C++ Possession Target State & Vessel Registration | 5 | Completed | DevOps | Track active possession state bidirectionally by adding PossessedTargetActor and PossessingEntity fields to USovereignSaveableEntityComponent. | Serialized Identity JSON block contains PossessedTargetActorName when possessed, restoring 3D playspace narrative coherence. |
| AD-025 | Sovereign SOP: Iron Knight Simulation Action Directive Protocol | 3 | Completed | Research/DevOps | Draft SOP protocol governing runtime simulation action directives, action capability manifests, and JSON schemas. | Created `AI_Nexus/Protocols/IronKnight_Simulation_Action_SOP.md`. |
| AD-026 | Iron Knight Bridge: POST /v1/unreal/directive API Endpoint | 5 | Completed | DevOps | Dedicated FastAPI endpoint for sending synchronous runtime action directives to Unreal simulation entities. | `/v1/unreal/directive` FastAPI endpoint implemented in `Sovereign_Intelligence/IronOfficer/bridge.py`. |
| AD-027 | USovereignBridgeSubsystem C++ Action Router & Vessel Dispatcher | 8 | Completed | DevOps | C++ Directive Parser in USovereignBridgeSubsystem that dispatches commands to target vessel components/functions. | C++ action routing and dispatch logic added to `USovereignBridgeSubsystem`. |
