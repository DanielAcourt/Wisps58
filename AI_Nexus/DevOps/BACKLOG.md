...
| AD-017 | UE 5.8.1 Python MCP Tool Registration & Scripting Directory | 5 | Todo | Research/DevOps | Create `/Content/Python/` scripting directory and register a live sample tool script exposing editor automation methods to `SovereignMCPClient`. | `/WispsCPPVR/Content/Python/` created with `init_unreal.py` and `sovereign_mcp_tools.py` registering live editor tools accessible via `GET /v1/mcp/tools`. |
