...
| B-043 | Fix Duplicate Entity Subobject & Serialization | 3 | Todo | DevOps | `ASovereignBaseInteractable` creates a second `USovereignSaveableEntityComponent` subobject (`SovereignSoul`), causing duplicate registrations and double entries in JSON save files. | Standardize subobject naming to `SaveDataComponent` in `ASovereignBaseInteractable`, preventing dual component creation and double entity serialization. |
