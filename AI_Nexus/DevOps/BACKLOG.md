...
| B-045 | Unreal 5.8.1 MCP Server Plugin Integration & Gitignore Security | 3 | Todo | DevOps | Keep the MCP Server Plugin registered at engine-level to prevent project and git repository bloat. | The MCPServer plugin is listed as active in .uproject, and its local files are strictly ignored on git. |
| B-046 | Web Caching & Repository Bloat Elimination Audit | 3 | Todo | DevOps | Prevent transient Webcache, log, and crash directories from dynamically inflating repository size. | Complete .gitignore overhaul targeting transient web, logs, intermediate files, and purging cached objects. |
